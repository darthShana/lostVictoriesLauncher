 ...
 * @author dharshanar
 */
public class M4A2ShermanBlenderModel extends TankBlenderModel{
    private static Vector3f muzzelLocation = new Vector3f(0f, 2.75f, 6);
    ...
    public M4A2ShermanBlenderModel(String modelPath, float walkSpeed, Weapon weapon) {
    ...
    public CollisionShape getTurretShape() {
        CompoundCollisionShape compoundShape = new CompoundCollisionShape();
        BoxCollisionShape turret = new BoxCollisionShape(new Vector3f(modelBounds.x/10*6, .75f, modelBounds.z/10*4));
        compoundShape.addChildShape(turret, new Vector3f(0, 2.5f, 0));
        BoxCollisionShape barrel = new BoxCollisionShape(new Vector3f(.1f, .1f, 3.6f));
